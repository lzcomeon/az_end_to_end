...
def divide_region(np.ndarray[DTYPE_t, ndim=2] regions,
    ...
    for i in xrange(num_regions):
        lengths[0] = regions[i,2] - regions[i,0] + 1.0  # 399-0+1=400
        lengths[1] = regions[i,3] - regions[i,1] + 1.0  # 499-0+1=500
        
        min_ind = np.argmin(lengths)  # 0
        max_ind = 1 - min_ind  # 1
        
        # number of sub-regions along each axis and their length
        num_short = 2        
        l_short = lengths[min_ind] / 2  # 200
        # number of sub-regions along the longer side is naturally longer
        num_long = int(lengths[max_ind] / l_short)  # 500/200=2.5  int 2
        l_long = lengths[max_ind] / num_long  # 500/2 = 250
        
        num_blocks = num_short * num_long + (num_short - 1) * (num_long - 1)  # 2*2+(2-1)*(2-1)=5
        ...
        offset = num_short * num_long  # 2*2=4
        h_short = l_short/2  #200/2=100
        h_long = l_long/2  # 250/2=125
