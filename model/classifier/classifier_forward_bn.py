...
class cls_layer_bn(nn.Module):



    def __init__(self, spatial_scale):
        super(cls_layer_bn, self).__init__()
        ...
        self.cls_bn1 = nn.BatchNorm1d(4096, momentum=0.5)
        self.cls_fc2 = nn.Linear(4096, 4096)
        self.cls_relu2 = nn.ReLU(inplace=True)
        self.cls_bn2 = nn.BatchNorm1d(4096, momentum=0.5)
    ...
    def forward(self, features, rois):
        ...
        cls_bn1 = self.cls_bn1(cls_relu1)
        cls_fc2 = self.cls_fc2(cls_bn1)
        cls_relu2 = self.cls_relu2(cls_fc2)
        cls_bn2 = self.cls_bn2(cls_relu2)
        bbox = self.bbox(cls_bn2)
        cls_score = self.cls_score(cls_bn2)
